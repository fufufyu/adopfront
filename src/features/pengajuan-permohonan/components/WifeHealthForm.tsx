...
const formSchema = z.object({
  ...
  suratKeteranganKesehatanJiwa: z
    ...
    .refine((file) => file?.size <= MAX_FILE_SIZE, "Ukuran maksimal file adalah 5MB")
    .refine((file) => ACCEPTED_FILE_TYPES.includes(file?.type), "Format yang diterima: .jpg, .jpeg, .png, dan .pdf"),
  suratKeteranganAhliKandungan: z
    .any()
    .refine((file) => file?.size <= MAX_FILE_SIZE, "Ukuran maksimal file adalah 5MB")
    .refine((file) => ACCEPTED_FILE_TYPES.includes(file?.type), "Format yang diterima: .jpg, .jpeg, .png, dan .pdf"),
})

export default function WifeHealthForm({ onSubmit }: { onSubmit: (data: any) => void; isPreview: boolean }) {
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
        ...
        />
        <FileUpload
          label="Surat Keterangan Ahli Kandungan"
          accept=".jpg,.jpeg,.png,.pdf"
          onChange={(file) => form.setValue("suratKeteranganAhliKandungan", file)}
          error={form.formState.errors.suratKeteranganAhliKandungan?.message as string}
        />
      </form>
    </Form>
  )
